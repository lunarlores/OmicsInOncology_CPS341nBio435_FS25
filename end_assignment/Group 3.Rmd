...
ggplot(psme1_data, aes(x = Expression, fill = Response)) +
  ...
  labs(
    title = "Expression of PSME1 by Response Group",
    x = "Expression Level",
    y = "Count"
  ) +
  theme_minimal()
...
# function to plot expression histograms for a given gene
plot_expression_histogram <- function(gene) {
  PDdata %>%
    filter(Gene_name == gene) %>%
    ggplot(aes(x = Expression, fill = Response)) +
    geom_histogram(bins = 30, position = "stack") +
    labs(
      title = paste("Expression of", gene, "by Response Group"),
      x = "Expression Level",
      y = "Count"
    ) +
    theme_minimal()
...
# Example proteins to investigate (you can pick from head(t_tests, 20))
plot_expression_histogram("PSME1")
plot_expression_histogram("CD74")
plot_expression_histogram("HLA-A")
plot_expression_histogram("TOMM22")  # choose one with near-zero fold change
plot_expression_histogram("YBX1")    # choose one with negative fold change
...
top10_genes <- diseased_vs_healthy %>%
  filter(significance != "ns") %>%
  slice_min(order_by = p_value, n = 10) %>%
  pull(Gene_name)

# Prepare heatmap data with Response in the same tibble
heatmap_data <- PDdata %>%
  filter(Gene_name %in% top10_genes) %>%
  select(Sample, Gene_name, Expression, Response)

# Use the version-compatible tidyHeatmap syntax
heatmap_data %>%
  tidyHeatmap::heatmap(
    .row = Sample,
    .column = Gene_name,
  ...
  tidyHeatmap::annotation_tile(Response)
...
```{r create protein list for STRING}

# Extract top 1000 proteins by lowest p-value
top_1000_proteins <- t_tests %>%
  slice_min(order_by = p_value, n = 1000) %>%
  pull(Gene_name)

# Write to a text file
write(top_1000_proteins, file = "top1000_proteins.txt")
